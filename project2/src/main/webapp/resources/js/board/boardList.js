    ...
    if(thumbnailList.length > 0){ // 썸네일이 존재할 경우
        ...
        for(let th of thumbnailList){
            th.addEventListener("click", ()=>{
                ...
                // modal창에 show 클래스 없으면 추가 (있으면 삭제)
        ...
        modalClose.addEventListener("click", ()=>{
            // modal.classList.toggle("show"); // 없어지긴 하는데 이게 닫히는게 아니라 display none이 되는거래
...
// 검색한 경우 검색창에 검색 key, query 유지시키기
(()=>{
    ...
    if(select != null){ // 검색창이 존재할 때
        ...
        // select에 이전 검색한 key의 값과 일치하는 option태그에
        // selected 속성 추가
