...
public class ImageDeleteScheduling {
    ...
    */
	...
	// 일정 시간마다 실행되게 만들어주는 어노테이션
//	@Scheduled(fixedDelay = 10000) // 이전 작업이 끝난 후 10초 뒤에 수행
//	@Scheduled(fixedRate = 1000)  // 이전 작업이 시작 후 10초 뒤에 수행
...
//	@Scheduled(cron = "0/15 * * * * *") // 0초 부터 시작하여 15초마다
	...
	// 로그를 출력하는 객체 얻어오기
	// or.slf4j.Logger로 import 하기
	// or.slf4j.LoggerFactory로 import 하기
	private Logger logger = LoggerFactory.getLogger(ImageDeleteScheduling.class);
	
	@Scheduled(cron = "0 * * * * *") // 매 분 0초마다 / 서버가 켜져 있을 때만 돌아간데.
	public void deleteImageFile() { // 매개변수는 아무것도 적을 수 없데
//		System.out.println("0초마다 출력");
		
		// 1. DB에서 BOARD_IMG 테이블의 모든 이미지 변경명을 조회
		List<String> dbList = service.seletImageList();
...
//				file.toString(); // 실제 파일이 저장된 경로 반환
				// 경로가 길어서 맨 뒤에 이름만 받아올거래 -> lastOfIndex로
				// 문자열.substring(인덱스) : 지정된 인덱스 부터 끝가지 잘라서 반환
				
				String fileName 
//				= file.toString().substring(file.toString().lastIndexOf("\\")+1);
				= file.getName();
				
				// /파일명.jsp 이렇게 나온데. / 한 번더 자르기 위해서 +1을 해준거
...
//					System.out.println(fileName = " 삭제");
					...
					// trace, debug, info, warn, error, fatal (주로 debug, info, warn, error를 씀)
