...
// 해당 클래스가 필터로 등록되게 해주고 요청 주소를 매핑
// 필터 등록 + 필터링할 주소 매핑

// 어노테이션 컨트롤러, 서비스, 리파짓토리에 접근을 못해서(어노테이션을 못지나서) 필터에서는 못쓴데
...
public class LoginFilter implements Filter {
	...
	public void destroy() {
		// 필터가 파괴될 때 / 서버 실행 중 필터 내용 변경 시 수행 후 init() 다시 수행
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if(session.getAttribute("loginMember") == null) { // 로그인 X
			...
			// 연결된 다음 필터로 이동(없으면 Servlet / JSP 로 이동)
