...
public class ChattingWebsocketHandler extends TextWebSocketHandler {
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		// + 웹소켓에 접속한 회원의 HTTPSession을 훔쳐서 가지고 있음(handshake-intercepter가 훔친거)
	...
	//  --> message.gePayLoad() == JSON데이터
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {

		// 전달 받은 내용 확인 (JSON) 
		// {"senderNo":"6","targetNo":"15","chattingNo":"1","messageContent":"채팅\n"}
		logger.info("전달 받은 내용 : " + message.getPayload());;
		
		// Jackson-databind에서 제공하는 ObjectMapper 객체 사용
		
		// ObjectMapper : JASON을 해석해서 지정된 VO로 변환하는 객체(필드에 알아서 값 세팅해줌)
